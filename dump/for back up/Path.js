const locations = {
  ...
  "Building 6": {
    type: "building",
    description: "Building 6 classrooms for Grade 12 students",
    directions: [],
    onSelect: function () {
      const fromSelect = document.getElementById("fromSelect");
      const from = fromSelect ? fromSelect.value : "";

      // Hide all arrow paths first
      document.querySelectorAll('[id^="From-"]').forEach((arrow) => {
        arrow.style.display = "none";
      });

      // Highlight Building 6
      const buildingShape = document.querySelector('[data-title="Building 6"]');
      if (buildingShape) buildingShape.classList.add("selected");

      // Set directions dynamically
      let directions = [];

      if (from === "main_gate") {
        directions = [
          "Start at the Main Gate.",
          "Follow the walkway heading left for about 92 meters.",
          "You’ll arrive at Building 6 on your left side.",
        ];
        ...
        directions = [
          "Start from the Canteen.",
          "Walk straight past the JHS Faculty area.",
          "Continue a few meters ahead — Building 6 will be right in front of you.",
        ];
        ...
        directions = [
          "Please select a starting point (Main Gate or Canteen) from the dropdown above to see directions to Building 6.",
        ];
      }

      // 🧭 Show the location name and info
      const locationInfo = document.getElementById("locationInfo");
      const selectedLocation = document.getElementById("selectedLocation");
      const locationDescription = document.getElementById(
        "locationDescription"
      );

      if (locationInfo && selectedLocation && locationDescription) {
        locationInfo.style.display = "block";
        selectedLocation.textContent = "Building 6";
        locationDescription.textContent =
          "This building contains Grade 12 classrooms.";
      }

      // 🗺️ Inject the directions into the right-hand panel
      ...
      if (directionsContainer) {
        directionsContainer.innerHTML = directions
          ...
          <div class="direction-step">
            <i class="fas fa-arrow-right me-2 text-primary"></i>${step}
          </div>`
          )
          .join("");
      }
